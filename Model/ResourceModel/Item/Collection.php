...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    /**
     * Add days in SaveForLater filter of product collection
     *
     * @var boolean
     */
    protected $_addDaysInSaveForLater = false;
    ...
    /**
     * @var \Mdbhojwani\SaveForLater\Model\ResourceModel\Item\Option\CollectionFactory
     */
    ...
    {
        $this->_init(\Mdbhojwani\SaveForLater\Model\Item::class, \Mdbhojwani\SaveForLater\Model\ResourceModel\Item::class);
    ...
    protected function _assignOptions()
    {
        ...
        /* @var $optionCollection \Mdbhojwani\SaveForLater\Model\ResourceModel\Item\Option\Collection */
    ...
    protected function _assignProducts()
    {
        \Magento\Framework\Profiler::start(
            'SAVEFORLATER:' . __METHOD__,
            ['group' => 'SAVEFORLATER', 'method' => __METHOD__]
        );
        ...
        $this->_eventManager->dispatch(
            'saveforlater_item_collection_products_after_load',
            ['product_collection' => $productCollection]
        );

        $checkInStock = $this->_productInStock && !$this->stockConfiguration->isShowOutOfStock();

        /** @var \Mdbhojwani\SaveForLater\Model\Item $item */
        foreach ($this as $item) {
        ...
        \Magento\Framework\Profiler::stop('SAVEFORLATER:' . __METHOD__);
    ...
    /**
     * Add filter by SaveForLater object
     *
     * @param \Mdbhojwani\SaveForLater\Model\SaveForLater $saveForLater
     * @return $this
     */
    public function addSaveForLaterFilter(\Mdbhojwani\SaveForLater\Model\SaveForLater $saveForLater)
    {
        $this->addFieldToFilter('saveforlater_id', $saveForLater->getId());
        return $this;
    }
    ...
    {
        $this->getSelect()->join(
            ['saveForLater' => $this->getTable('saveforlater')],
            'main_table.saveforlater_id = saveforlater.saveforlater_id',
            []
        )->where(
            'saveforlater.customer_id = ?',
            $customerId
        );
        return $this;
    }
    ...
    /**
     * Set flag of adding days in SaveForLater
     *
     * @return $this
     */
    public function addDaysInSaveForLater()
    {
        $this->_addDaysInSaveForLater = true;
        return $this;
    }

    /**
     * Adds filter on days in SaveForLater
     *
    ...
    public function getItemsQty()
    {
        if ($this->_itemsQty === null) {
            $this->_itemsQty = 0;
            foreach ($this as $saveForLaterItem) {
                $qty = $saveForLaterItem->getQty();
    ...
    protected function _afterLoadData()
    {
        ...
        if ($this->_addDaysInSaveForLater) {
            $gmtOffset = (int)$this->_date->getGmtOffset();
            $nowTimestamp = $this->_date->timestamp();

            foreach ($this as $saveForLaterItem) {
                $saveForLaterItemTimestamp = $this->_date->timestamp($saveForLaterItem->getAddedAt());

                $saveForLaterItem->setDaysInSaveForLater(
                    (int)(($nowTimestamp - $gmtOffset - $saveForLaterItemTimestamp) / 24 / 60 / 60)
                );
            }
        }
