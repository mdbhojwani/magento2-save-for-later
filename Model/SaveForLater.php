...
use Mdbhojwani\SaveForLater\Model\ResourceModel\Item\CollectionFactory;
use Mdbhojwani\SaveForLater\Model\ResourceModel\SaveForLater as ResourceSaveForLater;
use Mdbhojwani\SaveForLater\Model\ResourceModel\SaveForLater\Collection;

/**
 * SaveForLater model
 *
 * @method int getShared()
 * @method SaveForLater setShared(int $value)
 * @method string getSharingCode()
 * @method SaveForLater setSharingCode(string $value)
 * @method string getUpdatedAt()
 * @method SaveForLater setUpdatedAt(string $value)
 * @SuppressWarnings(PHPMD.CouplingBetweenObjects)
 * @SuppressWarnings(PHPMD.TooManyFields)
 */
class SaveForLater extends AbstractModel implements IdentityInterface
{
    /**
     * SaveForLater cache tag name
     */
    public const CACHE_TAG = 'saveforlater';
     ...
     * @var string
     */
    protected $_eventPrefix = 'saveforlater';

    /**
     * SaveForLater item collection
     *
     ...
     * @var ItemFactory
     */
    protected $_saveForLaterItemFactory;

    /**
     * @var CollectionFactory
     */
    protected $_saveForLaterCollectionFactory;
     ...
     * @param ResourceSaveForLater $resource
     * @param Collection $resourceCollection
     * @param StoreManagerInterface $storeManager
     * @param DateTime\DateTime $date
     * @param ItemFactory $saveForLaterItemFactory
    ...
    public function __construct(
        ...
        ResourceSaveForLater $resource,
        Collection $resourceCollection,
        StoreManagerInterface $storeManager,
        DateTime\DateTime $date,
        ItemFactory $saveForLaterItemFactory,
        ...
        $this->_saveForLaterData = $saveForLaterData;
        $this->_storeManager = $storeManager;
        $this->_date = $date;
        $this->_saveForLaterItemFactory = $saveForLaterItemFactory;
        $this->_saveForLaterCollectionFactory = $saveForLaterCollectionFactory;
    ...
    /**
     * Load SaveForLater by customer id
     *
     * @param int $customerId
     * @param bool $create Create SaveForLater if don't exists
     *
     * @return $this
     */
    ...
    public function getName()
    {
        ...
        if ($name === null || !strlen($name)) {
            return $this->_saveForLaterData->getDefaultSaveForLaterName();
        }
        return $name;
    }
    ...
    /**
     * Set date of last update for SaveForLater
     *
     * @return $this
     */
    ...
    /**
     * Add catalog product object data to SaveForLater
     *
    ...
    protected function _addCatalogProduct(Product $product, $qty = 1, $forciblySetQty = false)
    {
        ...
        if ($item === null) {
            $storeId = $product->hasSaveForLaterStoreId() ? $product->getSaveForLaterStoreId() : $this->getStore()->getId();
            $item = $this->_saveForLaterItemFactory->create();
            $item->setProductId($product->getId());
            $item->setSaveforlaterId($this->getId());
    ...
    /**
     * Retrieve SaveForLater item collection
     *
    ...
    public function getItemCollection()
    {
        if ($this->_itemCollection === null) {
            $this->_itemCollection = $this->_saveForLaterCollectionFactory->create()->addSaveForLaterFilter(
                $this
            )->addStoreFilter(
    ...
    {
        $item->setSaveForLater($this);
        if (!$item->getId()) {
            $this->getItemCollection()->addItem($item);
            $this->_eventManager->dispatch('saveforlater_add_item', ['item' => $item]);
        }
        return $this;
    }

    /**
     * Adds new product to SaveForLater.
     *
    ...
    public function addNewItem($product, $buyRequest = null, $forciblySetQty = false)
    {
         ...
         * a) we have new instance and do not interfere with other products in SaveForLater
         * b) product has full set of attributes
         */
        if ($product instanceof Product) {
            $productId = $product->getId();
            // Maybe force some store by SaveForLater internal properties
            $storeId = $product->hasSaveForLaterStoreId() ? $product->getSaveForLaterStoreId() : $product->getStoreId();
        } else {
        ...
        if (!$this->stockConfiguration->isShowOutOfStock($storeId) && !$product->getIsSalable()) {
            throw new StockStateException(__('Cannot add product without stock to Save For Later.'));
        }
        ...
        if ($buyRequest instanceof DataObject) {
            ...
            if ($isInvalidItemConfiguration) {
                throw new InvalidArgumentException('Invalid Save For Later item configuration.');
            }
        ...
        foreach ($cartCandidates as $candidate) {
            ...
            $candidate->setSaveForLaterStoreId($storeId);
        ...
        $this->_eventManager->dispatch('saveforlater_product_add_after', ['items' => $items]);
    ...
    /**
     * Retrieve SaveForLater store object
     *
    ...
    /**
     * Retrieve SaveForLater items count
     *
     * @return int
     */
    ...
    /**
     * Retrieve SaveForLater has salable item(s)
     *
     * @return bool
     */
    ...
    /**
     * Check customer is owner this SaveForLater
     *
     * @param int $customerId
     *
     * @return bool
    ...
    /**
     * Update SaveForLater Item and set data from request
     *
    ...
    public function updateItem($itemId, $buyRequest, $params = null)
    {
        ...
        if (!$item) {
            throw new LocalizedException(__('We can\'t specify a Save For Later item.'));
        }
        ...
        if ($productId) {
            ...
            $product->setSaveForLaterStoreId($item->getStoreId());
            $items = $this->getItemCollection();
            $isForceSetQuantity = true;
            foreach ($items as $saveForLaterItem) {
                /* @var $saveForLaterItem Item */
                if ($saveForLaterItem->getProductId() == $product->getId()
                    && $saveForLaterItem->getId() != $item->getId()
                    && $saveForLaterItem->representProduct($product)
                ) {
                    // We do not add new SaveForLater item, but updating the existing one
